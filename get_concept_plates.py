...
def fetch_plates(fs_code, plate_type, name, max_pages=10):
    subdomains = ["push2", "12.push2", "13.push2", "20.push2", "27.push2", 
                  "56.push2", "38.push2", "48.push2", "79.push2", "25.push2", "62.push2"]
    ...
    while page <= max_pages:
        ...
        for attempt in range(3):   # 每页最多重试3次
            ...
            if page_success:
                break
            time.sleep(random.uniform(2.0, 4.0))   # 失败后多等一会儿
        
        if not page_success:
            print(f"  【{name}】 第 {page} 页多次失败，跳过...")
        
        page += 1
        time.sleep(random.uniform(1.5, 3.0))   # 加大页间间隔
