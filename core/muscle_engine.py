...
class MuscleEngine:
    ...
    # --- 阶段一：Playwright 幽灵扫描 (借壳生蛋版) ---
    
    async def fetch_dynamic_sector_list(self) -> list:
        logger.info("🧠 [Phase 1] 启动浏览器“借壳生蛋”模式，深度渗透全量目录...")
        all_codes = set()
        # 将 PZ 提升到 250，减少翻页次数，降低被封概率
        ...
        async with async_playwright() as p:
            ...
            try:
                # 💡 1. 预热：先访问官方行情页面，建立合法上下文
                logger.info("🌐 正在预热浏览器上下文 (访问行情中心)...")
                await page.goto("https://quote.eastmoney.com/center/gridlist.html#boards", wait_until="networkidle")
                await asyncio.sleep(2)

                for cat_name, fs_param in categories.items():
                    logger.info(f"➡️ 正在扫描分类: {cat_name}")
                    ...
                    for pn in range(1, 15): 
                        ...
                        # 💡 2. 借壳请求：在页面内执行 fetch，这是目前最完美的伪装
                        fetch_js = f"fetch('{target_url}').then(r => r.text())"
                        try:
                            content = await page.evaluate(fetch_js)
                            data = self._extract_json(content)
                            
                            if data and data.get("data") and data["data"].get("diff"):
                                diff = data["data"]["diff"]
                                for x in diff: all_codes.add(f"90.{x['f12']}")
                                logger.debug(f"✅ {cat_name} P{pn} 捕获 {len(diff)} 个")
                                if len(diff) < 100: break
                            else:
                                logger.warning(f"⚠️ {cat_name} P{pn} 返回为空，分类扫描提前结束")
                                break
                        except Exception as e:
                            logger.error(f"❌ 借壳请求异常: {str(e)[:50]}")
                            break
        ...
        logger.success(f"🧠 [Phase 1] 渗透大获全胜！最终捕获 {len(all_codes)} 个唯一板块 ID。")
        return list(all_codes)

    # --- 阶段二：curl_cffi 并发抓取 (已验证稳态) ---
    ...
    async def fetch_all_sectors(self, sector_list: list):
        logger.info(f"💪 [Phase 2] 启动 curl_cffi 并发引擎 | 并发数: {self.concurrency}")
